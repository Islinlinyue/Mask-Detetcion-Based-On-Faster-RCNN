...
def main():
    ...
    VOC_root = "/home/featurize/myproject"  # VOCdevkit
    ...
    for epoch in range(init_epochs):
        ...
        mean_loss_val = utils.train_one_epoch(model, optimizer, val_data_loader,
                                              device, epoch, print_freq=50,
                                              warmup=True, scaler=scaler)[0]


        val_loss.append(mean_loss_val.item())

        # # evaluate on the test dataset
        # coco_info_val = utils.evaluate(model, val_data_loader, device=device)
        ...
        # val_map.append(coco_info_val[1])  # pascal mAP

    torch.save(model.state_dict(), "./save_weights/vgg_pretrain.pth")
    ...
    if len(train_loss) != 0 and len(val_loss) != 0 and len(learning_rate) != 0:
        from plot_curve import plot_loss_and_lr
        plot_loss_and_lr(train_loss, val_loss, learning_rate)
