...
def main(parser_data):
    ...
    optimizer = torch.optim.SGD(params, lr=0.005, #0.005, 0.001, 0.0001
                                momentum=0.9, weight_decay=0.0005)

    # optimizer = torch.optim.ASGD(params, lr=0.005, weight_decay=0.0005)
    ...
    for epoch in range(parser_data.start_epoch, parser_data.epochs):
        ...
        mean_loss_val = utils.train_one_epoch(model, optimizer, val_data_set_loader,
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default="/root/autodl-tmp/project", help='dataset')
    # 检测目标类别数(不包含背景)
